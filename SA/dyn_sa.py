...
def dyn_init_SA(config,
    ...
    ############################################
    dyn_SA_config_file = os.path.join(
        get_plugin_path("FabDynamics"),
        "SA",
        "dyn_SA_config.yml"
    )
    dyn_SA_campaign_config = load_dyn_SA_campaign_config(dyn_SA_config_file)
    polynomial_order = dyn_SA_campaign_config["polynomial_order"]

    if sampler_name is None:
        sampler_name = dyn_SA_campaign_config["sampler_name"]

    ############################
    # Create easyvvuq campaign #
    ############################
    campaign_name = "dyn_SA_{}".format(sampler_name)
    campaign_work_dir = os.path.join(
        get_plugin_path("FabDynamics"),
        "SA",
        "dyn_SA_{}".format(sampler_name)
    )
...
@task
@load_plugin_env_vars("FabDynamics")
def dyn_analyse_SA(config, sampler_name=None, ** args):
    """
    ===========================================================================
    ...
    #####################################################
    output_filename = dyn_SA_campaign_config[
    ...
    #######################################
    output_column = dyn_SA_campaign_config["decoder_output_column"]
    ...
    with open(dyn_SA_config_file, 'r') as ff:
    ...
    yml_results["campaign_info"].update({
        ...
        "polynomial_order": polynomial_order,
        "sampler": S(dyn_SA_campaign_config["sampler_name"]),
        "distribution_type": S(dyn_SA_campaign_config["distribution_type"]),
        "sparse": S(dyn_SA_campaign_config["sparse"]),
        "growth": S(dyn_SA_campaign_config["growth"])
    })
    if sampler_name == "SCSampler":
        yml_results["campaign_info"].update({
            "quadrature_rule": S(dyn_SA_campaign_config["quadrature_rule"]),
            "midpoint_level1": S(dyn_SA_campaign_config["midpoint_level1"]),
            "dimension_adaptive": S(dyn_SA_campaign_config["dimension_adaptive"])
        })
    elif sampler_name == "PCESampler":
        yml_results["campaign_info"].update({
            "rule": S(dyn_SA_campaign_config["quadrature_rule"]),
        })

    ROUND_NDIGITS = 4
    for param in dyn_SA_campaign_config["selected_vary_parameters"]:
...
def load_dyn_SA_campaign_config(facs_SA_config_file):
    ...
    #####################################################
    sampler_params_json_PATH = os.path.join(get_plugin_path("FabDynamics"),
                                            "templates",
                                            "params.json"
                                            )
...
def init_dyn_SA_campaign(campaign_name, campaign_config,
    ...
    encoder = uq.encoders.MultiEncoder(
        ...
        uq.encoders.GenericEncoder(
            template_fname=get_plugin_path("FabDynamics") +
            "/templates/" + template,
            delimiter="$",
